...
class SoundAQnet_S1(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        ...
        super(SoundAQnet_S1, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        event_embs_loudness = x_k_3_loudness
        ...
        loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 1, :][None, :]], dim=-1)))
        # loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 5, :][None, :]], dim=-1)))
        # loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 6, :][None, :]], dim=-1)))
        # loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 7, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k3, loudness_k3], dim=0)
...
class SoundAQnet_S2(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        ...
        super(SoundAQnet_S2, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = x_k_5_loudness
        ...
        # mel_k3 = F.gelu(self.fc_residual_mel_k3(torch.cat([x_k_3_mel, x[:, 0, :][None, :] ], dim=-1)))
        mel_k5 = F.gelu(self.fc_residual_mel_k5(torch.cat([x_k_5_mel, x[:, 0, :][None, :]], dim=-1)))
        # mel_k7 = F.gelu(self.fc_residual_mel_k7(torch.cat([x_k_7_mel, x[:, 2, :][None, :]], dim=-1)))
        # mel_k9 = F.gelu(self.fc_residual_mel_k9(torch.cat([x_k_9_mel, x[:, 3, :][None, :]], dim=-1)))

        # loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 4, :][None, :]], dim=-1)))
        loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 1, :][None, :]], dim=-1)))
        # loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 6, :][None, :]], dim=-1)))
        # loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 7, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k5, loudness_k5], dim=0)
...
class SoundAQnet_S3(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        ...
        super(SoundAQnet_S3, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = x_k_7_loudness
        ...
        # mel_k3 = F.gelu(self.fc_residual_mel_k3(torch.cat([x_k_3_mel, x[:, 0, :][None, :] ], dim=-1)))
        # mel_k5 = F.gelu(self.fc_residual_mel_k5(torch.cat([x_k_5_mel, x[:, 0, :][None, :]], dim=-1)))
        mel_k7 = F.gelu(self.fc_residual_mel_k7(torch.cat([x_k_7_mel, x[:, 0, :][None, :]], dim=-1)))
        # mel_k9 = F.gelu(self.fc_residual_mel_k9(torch.cat([x_k_9_mel, x[:, 3, :][None, :]], dim=-1)))

        # loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 4, :][None, :]], dim=-1)))
        # loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 1, :][None, :]], dim=-1)))
        loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 1, :][None, :]], dim=-1)))
        # loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 7, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k7, loudness_k7], dim=0)
...
class SoundAQnet_S4(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        ...
        super(SoundAQnet_S4, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = x_k_9_loudness
        ...
        # mel_k5 = F.gelu(self.fc_residual_mel_k5(torch.cat([x_k_5_mel, x[:, 0, :][None, :]], dim=-1)))
        # mel_k7 = F.gelu(self.fc_residual_mel_k7(torch.cat([x_k_7_mel, x[:, 0, :][None, :]], dim=-1)))
        mel_k9 = F.gelu(self.fc_residual_mel_k9(torch.cat([x_k_9_mel, x[:, 0, :][None, :]], dim=-1)))

        # loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 4, :][None, :]], dim=-1)))
        # loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 1, :][None, :]], dim=-1)))
        # loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 1, :][None, :]], dim=-1)))
        loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 1, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k9, loudness_k9], dim=0)
...
class SoundAQnet_S1_S2(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        hidden_dim = 32,
        out_dim = 64,
        n_layers = 1,
        ...
        super(SoundAQnet_S1_S2, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_log_mel = torch.cat([x_k_3_mel, x_k_5_mel, x_k_7_mel, x_k_9_mel], dim=0)
        event_embs_log_mel = torch.cat([x_k_3_mel, x_k_5_mel], dim=0)
        # print(event_embs_log_mel.size())  # torch.Size([4, 16, 64])  (node_num, batch, edge_dim)
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness], dim=0)
        ...
        loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 2, :][None, :]], dim=-1)))
        loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 3, :][None, :]], dim=-1)))
        # loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 1, :][None, :]], dim=-1)))
        # loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 1, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k3, mel_k5, loudness_k3, loudness_k5], dim=0)
...
class SoundAQnet_S1_S2_S3(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        hidden_dim = 32,
        out_dim = 64,
        n_layers = 1,
        ...
        super(SoundAQnet_S1_S2_S3, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_log_mel = torch.cat([x_k_3_mel, x_k_5_mel, x_k_7_mel, x_k_9_mel], dim=0)
        event_embs_log_mel = torch.cat([x_k_3_mel, x_k_5_mel, x_k_7_mel], dim=0)
        # print(event_embs_log_mel.size())  # torch.Size([4, 16, 64])  (node_num, batch, edge_dim)
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness], dim=0)
        ...
        loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 3, :][None, :]], dim=-1)))
        loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 4, :][None, :]], dim=-1)))
        loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 5, :][None, :]], dim=-1)))
        # loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 1, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, loudness_k3, loudness_k5, loudness_k7], dim=0)
...
class SoundAQnet_S3_S4(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        hidden_dim = 32,
        out_dim = 64,
        n_layers = 1,
        ...
        super(SoundAQnet_S3_S4, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_log_mel = torch.cat([x_k_3_mel, x_k_5_mel, x_k_7_mel, x_k_9_mel], dim=0)
        event_embs_log_mel = torch.cat([x_k_7_mel, x_k_9_mel], dim=0)
        # print(event_embs_log_mel.size())  # torch.Size([4, 16, 64])  (node_num, batch, edge_dim)
        ...
        # -------------------------------------------------------------------------------------------------------------
        # event_embs_loudness = torch.cat([x_k_3_loudness, x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        event_embs_loudness = torch.cat([x_k_7_loudness, x_k_9_loudness], dim=0)
        ...
        # mel_k5 = F.gelu(self.fc_residual_mel_k5(torch.cat([x_k_5_mel, x[:, 0, :][None, :]], dim=-1)))
        mel_k7 = F.gelu(self.fc_residual_mel_k7(torch.cat([x_k_7_mel, x[:, 0, :][None, :]], dim=-1)))
        mel_k9 = F.gelu(self.fc_residual_mel_k9(torch.cat([x_k_9_mel, x[:, 1, :][None, :]], dim=-1)))

        # loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 4, :][None, :]], dim=-1)))
        # loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 1, :][None, :]], dim=-1)))
        loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 2, :][None, :]], dim=-1)))
        loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 3, :][None, :]], dim=-1)))

        # kernels_embs = torch.cat([mel_k3, mel_k5, mel_k7, mel_k9, loudness_k3, loudness_k5, loudness_k7, loudness_k9], dim=0)
        kernels_embs = torch.cat([mel_k7, mel_k9, loudness_k7, loudness_k9], dim=0)
...
class SoundAQnet_S2_S3_S4(nn.Module):
    def __init__(self, max_node_num, node_emb_dim = 256,
        hidden_dim = 32,
        out_dim = 64,
        n_layers = 1,
        ...
        super(SoundAQnet_S2_S3_S4, self).__init__()
    ...
    def forward(self, input, batch_x_loudness, batch_graph):
        ...
        # -------------------------------------------------------------------------------------------------------------
        event_embs_log_mel = torch.cat([x_k_5_mel, x_k_7_mel, x_k_9_mel], dim=0)
        # print(event_embs_log_mel.size())  # torch.Size([4, 16, 64])  (node_num, batch, edge_dim)
        ...
        # -------------------------------------------------------------------------------------------------------------
        event_embs_loudness = torch.cat([x_k_5_loudness, x_k_7_loudness, x_k_9_loudness], dim=0)
        ...
        # mel_k3 = F.gelu(self.fc_residual_mel_k3(torch.cat([x_k_3_mel, x[:, 0, :][None, :] ], dim=-1)))
        mel_k5 = F.gelu(self.fc_residual_mel_k5(torch.cat([x_k_5_mel, x[:, 0, :][None, :]], dim=-1)))
        mel_k7 = F.gelu(self.fc_residual_mel_k7(torch.cat([x_k_7_mel, x[:, 1, :][None, :]], dim=-1)))
        mel_k9 = F.gelu(self.fc_residual_mel_k9(torch.cat([x_k_9_mel, x[:, 2, :][None, :]], dim=-1)))

        # loudness_k3 = F.gelu(self.fc_residual_loudness_k3(torch.cat([x_k_3_loudness, x[:, 4, :][None, :]], dim=-1)))
        loudness_k5 = F.gelu(self.fc_residual_loudness_k5(torch.cat([x_k_5_loudness, x[:, 3, :][None, :]], dim=-1)))
        loudness_k7 = F.gelu(self.fc_residual_loudness_k7(torch.cat([x_k_7_loudness, x[:, 4, :][None, :]], dim=-1)))
        loudness_k9 = F.gelu(self.fc_residual_loudness_k9(torch.cat([x_k_9_loudness, x[:, 5, :][None, :]], dim=-1)))

        kernels_embs = torch.cat([ mel_k5, mel_k7, mel_k9, loudness_k5, loudness_k7, loudness_k9], dim=0)
