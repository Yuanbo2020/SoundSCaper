...
class TransDecoderModel(AACModel):
    def __init__(
        ...
        self.linear1: nn.Module = nn.Identity()
    ...
    def configure_model(self) -> None:
        ...
        linear1 = nn.Sequential(
            nn.Linear(64, self.hparams["in_features"]),
            Transpose(1, 2),
        )
        ...
        # linear1 = nn.Linear(64, 768)
        # projection = nn.Sequential(
        #     nn.Dropout(p=0.5),
        #     # Transpose(1, 2),                     # [B, T, 64] → [B, 64, T]
        #     nn.Linear(64, 768),                  # 新增适配层：64 → 768
        #     nn.ReLU(inplace=True),               # 可选激活
        #     nn.Linear(768, self.hparams["d_model"]),  # 原线性层：768 → 256
        #     Transpose(1, 2),                     # [B, 256, T] → [B, T, 256]
        #     nn.Dropout(p=0.5),
        # )
        ...
        # print("vocab_size", self.tokenizer.get_vocab_size())
    ...
    def encode_audio(
        ...
        # remove channel dim
        if frame_embs_shape.dim() == 1:
            frame_embs_shape = frame_embs_shape.unsqueeze(0)  # make it (1, n)  
        # frame_embs_shape = frame_embs_shape.unsqueeze(dim=0)
        frame_embs_lens = frame_embs_shape[:, time_dim]
        # frame_embs_lens: (bsize,)
        # linear1
        # print("frame_embs", frame_embs.shape)
        frame_embs = self.linear1(frame_embs)
        # print("frame_embs", frame_embs.shape)
