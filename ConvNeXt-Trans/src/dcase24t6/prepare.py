...
from dcase24t6.datamodules.custom import CustomDataset
...
def prepare(cfg: DictConfig) -> None:
    ...
    hdf_datasets = prepare_custom_hdf_datasets(
        dataroot=cfg.path.data_root,
        subsets=cfg.data.subsets,
        hdf_pattern=cfg.hdf_pattern,
        pre_process=pre_process,
...
def prepare_custom_hdf_datasets(
    ...
    log_mel_path = os.path.join(dataroot, "training_log_mel.pickle")
    captions_path = os.path.join(dataroot, "captions.json")
    subset_path = os.path.join(dataroot, "subset.json")
    assert os.path.exists(log_mel_path), f"{log_mel_path} not found."
    assert os.path.exists(captions_path), f"{captions_path} not found."
...
def prepare_data_metrics_models(
    ...
    hdf_datasets = {}
    dataset_name = "clotho"
    pylog.info(f"Preparing {dataset_name} datasets")
    for subset in subsets:
        dataset = Clotho(
            root=dataroot,
            subset=subset,
